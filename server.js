...
const STATUS_COLORS = {
  ...
  in_transit: 0xd97706, delivered: 0x15803d, exception: 0xb91c1c,
};
const STATUS_LABELS = {
...
function notifyJobEvent(job, actorName, extra = '') {
  const isAlert = job.status === 'exception';
...
// Only the milestones a customer actually cares about, in plain language.
const CUSTOMER_MILESTONES = {
  received:    { title: '📦 Order received', body: (o) => `We've got your order and it's in the queue.`, color: 0x534ab7 },
  preparing:   { title: '🔧 Preparing your order', body: (o) => `Our team is picking and packing your items.`, color: 0xb45309 },
  out:         { title: '🚚 Out for delivery', body: (o, d) => `${d ? d + ' is' : 'A driver is'} on the way to ${o.destination}.`, color: 0xd97706 },
  delivered:   { title: '✅ Delivered', body: (o) => `Your order reached ${o.destination}. Thanks!`, color: 0x15803d },
  delayed:     { title: '⏳ Delivery delayed', body: (o, d, note) => note ? `Heads up: ${note}` : `There's a short delay. We're on it.`, color: 0xb91c1c },
};
...
/* ---------- Jobs ---------- */
const JOB_FLOW = { picking: 'packed', packed: 'assigned', assigned: 'in_transit', in_transit: 'delivered' };
...
app.patch('/api/jobs/:id/assign', auth('warehouse', 'admin'), (req, res) => {
  ...
  if (job.status !== 'packed') return res.status(400).json({ error: 'Job must be packed before assigning a driver' });
...
app.patch('/api/jobs/:id/status', auth(), (req, res) => {
  ...
  if (req.user.role === 'driver') {
    ...
    const driverAllowed = (job.status === 'assigned' && status === 'in_transit')
      || (job.status === 'in_transit' && status === 'delivered')
      || status === 'exception';
    if (!driverAllowed) return res.status(400).json({ error: 'Drivers can start transit, deliver, or flag an exception' });
  } else {
    const valid = JOB_FLOW[job.status] === status || status === 'exception'
      || (job.status === 'exception' && ['packed', 'assigned'].includes(status));
  ...
  if (status === 'in_transit') notifyCustomer('out', order, { driverName });
...
// Maps the latest job status to a customer-safe stage. No internal notes or driver IDs leak.
const PUBLIC_STAGE = {
  picking: 'preparing', packed: 'preparing', assigned: 'preparing',
  in_transit: 'out', delivered: 'delivered', exception: 'delayed',
};
...
app.get('/api/track/:ref', (req, res) => {
  ...
  const job = db.prepare('SELECT status, driver_id, notes, updated_at FROM jobs WHERE order_id = ? ORDER BY id DESC LIMIT 1').get(order.id);
  let stage = 'received';
  let delayNote = '';
  if (job) {
    stage = PUBLIC_STAGE[job.status] || 'received';
    if (job.status === 'exception') delayNote = job.notes || '';
  }
  res.json({
    ref: order.ref,
...
firstRun();
app.listen(PORT, () => console.log(`Waybill OS running on http://localhost:${PORT}`));
