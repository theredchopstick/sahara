...
Delete `waybill.db` to start over. (If you seed, the demo `admin` is used and the
...
```bash
docker build -t waybill .
docker run -p 3000:3000 -v waybill-data:/data waybill
```

The `-v waybill-data:/data` volume keeps the database between restarts, with no monthly
...
PORT=3000
DB_PATH=./waybill.db                   # set to /data/waybill.db on a persistent disk
```
